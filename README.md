...
- The felt adapts to the available viewport, with visible table limits and payout ratios. The header, table, and controls fit together without page scrolling, including phones, landscape screens, embedded frames, and fullscreen. Use **Fullscreen** to fill the display and **Escape** to exit; your hand stays in progress.
...
- **Undo**, **Clear**, **Rebet**, and **Remove** are on the bottom control rail. Remove toggles subtraction for touch screens; right-click also removes the selected denomination. **Edit amount** opens the exact-bet editor. Opening chips are deducted when you deal. Chips retain their denominations, automatically sorting from largest on the bottom to smallest on top, with distinct colored edges.
- Trips sits above Ante; Play sits below Ante. Winnings arrive as a complete, separate stack beside the original wager, pause on the felt, and then return to your wallet. Dealing and settlement run at a relaxed pace. A chip reserve beside the wallet grows and shrinks with the available balance, excluding money still waiting to be collected.
...
node tests/browser-fit.cjs
node tests/browser-redesign.cjs
node tests/browser-poker-feedback.cjs
...
The repository owner's workflow is to test changes and then push the complete project directly to `main`, including source files, tests, documentation, and assets. Preserve unrelated remote work and use normal fast-forward updates.
